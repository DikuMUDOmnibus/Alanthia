...
PC_DATA *new_pcdata(void)
{
    ...
    *pcdata = pcdata_zero;
    ...
    for (alias = 0; alias < MAX_CHANNELS; alias++)
	pcdata->channel[alias] = NULL;
...
void free_pcdata(PC_DATA * pcdata)
{
    ...
    free_string(pcdata->wrath);
    free_string(pcdata->title);
    free_string(pcdata->god);
    free_string(pcdata->bountier);
    free_buf(pcdata->buffer);

    for (alias = 0; alias < MAX_CHANNELS; alias++)
	free_string(pcdata->channel[alias]);
...
bool add_buf(BUFFER * buffer, char *string)
{
	...
	buffer->string = alloc_mem(buffer->size);
	//printf("buffer->size != oldsize\r\n");

	strcpy(buffer->string, oldstr);
	free_mem(oldstr);
    }
    strcat(buffer->string, string);
	//printf(" gonna return TRUE from add_buf\r\n");
    return TRUE;
}
