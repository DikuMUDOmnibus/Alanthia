...
/* local procedures */
void load_thread(char *name, NOTE_DATA ** list, int type,
		 time_t free_time);
void parse_note(CHAR_DATA * ch, char *argument, int type);
bool hide_note(CHAR_DATA * ch, NOTE_DATA * pnote);

NOTE_DATA *qnote_list;
...
void do_unread(CHAR_DATA * ch)
{
    ...
    if ((count = count_spool(ch, qnote_list)) > 0) {
	found = TRUE;
	sprintf(buf,
		"{mThere %s %d new qwest article%s waiting. (qnote){x\n\r",
	...
	found = TRUE;
	sprintf(buf,
		"{bThere %s %d new roleplay article%s waiting. (rpnote){x\n\r",
		count > 1 ? "are" : "is", count, count > 1 ? "s" : "");
	send_to_char(buf, ch);
    }
	...
	found = TRUE;
	sprintf(buf,
		"{yThere %s %d new news article%s waiting. (news){x\n\r",
		count > 1 ? "are" : "is", count, count > 1 ? "s" : "");
	send_to_char(buf, ch);
    }
	...
	found = TRUE;
	sprintf(buf,
		"{gThere %s %d change%s waiting to be read. (change){x\n\r",
		count > 1 ? "are" : "is", count, count > 1 ? "s" : "");
	send_to_char(buf, ch);
    }
    if ((count = count_spool(ch, note_list)) > 0) {
	found = TRUE;
	sprintf(buf, "{WYou have %d new note%s waiting. (note){x\n\r",
		count, count > 1 ? "s" : "");
	send_to_char(buf, ch);
    }
    if ((count = count_spool(ch, idea_list)) > 0) {
	found = TRUE;
	sprintf(buf, "{cYou have %d unread idea%s to peruse. (idea){x\n\r",
		count, count > 1 ? "s" : "");
	send_to_char(buf, ch);
    }
	...
	found = TRUE;
	sprintf(buf, "{r%d %s been added. (penalty){x\n\r",
...
void do_qnote(CHAR_DATA * ch, char *argument)
{
    parse_note(ch, argument, NOTE_QNOTE);
}
...
void load_notes(void)
{
    ...
    load_thread(QNOTE_FILE, &qnote_list, NOTE_QNOTE, 0);
...
bool is_note_to(CHAR_DATA * ch, NOTE_DATA * pnote)
{
    ...
    if (IS_SET(ch->comm, PLR_LEADER) && is_name("leaders", pnote->to_list))
	return TRUE;

    if (IS_SET(ch->comm, PLR_DEL) && is_name("delegates", pnote->to_list))
	return TRUE;

    if (ch->clan
	&& is_exact_name(clan_table[ch->clan].name,
			 pnote->to_list)) return TRUE;
...
void parse_note(CHAR_DATA * ch, char *argument, int type)
{
	...
	if (!vnum) {
		...
		break;
	    case NOTE_QNOTE:
		send_to_char("There are no qwest notes for you.\n\r", ch);
		break;
	    case NOTE_RPNOTE:
...
/* wdl, 4.10.98, closing the door on newbie note postings! */

	if (ch->level <= 1) {
	    send_to_char
		("\r\nSorry, you must be level two to post to Alanthia's note system.\r\n",
		 ch);
	    return;
	}
	note_attach(ch, type);
...
/* wdl, 4.10.98, closing the door on newbie note postings! */
	...
	sprintf(buf, "{c[{RANNOUNCE{c] $n just posted %s!{x",
		ch->pnote->type == NOTE_NOTE ? "a note" :
		ch->pnote->type == NOTE_IDEA ? "an idea" :
		ch->pnote->type == NOTE_PENALTY ? "a penalty" :
		ch->pnote->type == NOTE_QNOTE ? "a qwest note" :
		ch->pnote->type == NOTE_RPNOTE ? "an rp note" :
		ch->pnote->type == NOTE_NEWS ? "some news" : "a change");
	...
	wiznet("New note posted.", ch, NULL, 0, 0, 0);
