...
void fwrite_char(CHAR_DATA * ch, FILE * fp)
{
    ...
    fprintf(fp, "Not  %ld %ld %ld %ld %ld %ld %ld\n",
	    ...
	    ch->pcdata->last_changes, ch->pcdata->last_qnote,
	    ch->pcdata->last_rpnote);
    fprintf(fp, "Scro %d\n", ch->lines);
    fprintf(fp, "Room %ld\n",
    ...
    fprintf(fp, "HMV  %ld %ld %ld %ld %ld %ld\n",
	    ...
	    ch->max_move);
    /*put here for a lame hack to make ch->max_blood 
       be calculated every login */
    fprintf(fp, "Rem %d\n", ch->remorts);
    fprintf(fp, "Blood %d\n", ch->blood);
    if (ch->gold > 0)
	fprintf(fp, "Gold %ld\n", ch->gold);
	...
	if (ch->pcdata->bountier[0] != '\0')
	    fprintf(fp, "Bountier %s~\n", ch->pcdata->bountier);
	...
	if (!IS_NPC(ch)) {
	    /* comments because of a lame hack to make bp be calculated 
	       every login */
	    /*            fprintf(fp, "Rem %d\n", ch->remorts); */
	    fprintf(fp, "Slo %d\n", ch->slots);
	}
...
/* End ignore command code */

	for (pos = 0; pos < MAX_ATTACKS; pos++) {
	    fprintf(fp, "Attack  %d %d\n", pos, ch->attack_stack[pos]);
		if (ch->attack_stack[pos] == -1)
			break;
	}
	...
	for (sn = 0; sn < MAX_SKILL; sn++) {
	    if (skill_table[sn].name != NULL && ch->pcdata->slot[sn] > 0) {
		fprintf(fp, "Sl %d '%s'\n", ch->pcdata->slot[sn],
			skill_table[sn].name);
	    }
	}
...
void fwrite_obj(CHAR_DATA * ch, OBJ_DATA * obj, FILE * fp, int iNest)
{
    ...
    case ITEM_PILL:
	if (obj->value[1] > 0) {
	    if (skill_table[obj->value[1]].name != NULL) {
		fprintf(fp, "Spell 1 '%s'\n",
			skill_table[obj->value[1]].name);
	    } else
		fprintf(fp, "Spell 1 'Haste'\n");
...
bool load_char_obj(DESCRIPTOR_DATA * d, char *name)
{
    ...
    ch->pcdata->wrath = str_dup("");
    ch->pcdata->title = str_dup("");
    ch->pcdata->god = str_dup("");
    ch->pcdata->bountier = str_dup("");
    ch->attack_stack[0] = -1;
	...
	ch->pcdata->channel[i] = str_dup(color_table[i].color);
    ...
    //some hardcoded flags to combat buffer overflows
    SET_BIT(ch->comm, COMM_COMBINE);

    if (ch->lines == 0)
	ch->lines = 100;
    // end buffer overflow combat
...
void fread_char(CHAR_DATA * ch, FILE * fp)
{
	...
	case 'A':
	    ...
	    if (!str_cmp(word, "Attack")) {
			int pos = 0;
			pos = fread_number(fp);
			ch->attack_stack[pos] = fread_number(fp);
			fMatch = TRUE;
			break;
	    }
	...
	case 'B':
	    ...
	    KEY("Bal", ch->pcdata->balance, fread_number(fp));
	    KEY("Bamfout", ch->pcdata->bamfout, fread_string(fp));
	    KEY("Bin", ch->pcdata->bamfin, fread_string(fp));
	    KEY("Bounty", ch->pcdata->bounty, fread_number(fp));
	    KEY("Bountier", ch->pcdata->bountier, fread_string(fp));
	    KEY("Bout", ch->pcdata->bamfout, fread_string(fp));
	    if (!str_cmp(word, "BloodP")) {
		fread_number(fp);
		...
		ch->max_blood = ch->level + (20 * (ch->remorts + 1));
		ch->pcdata->perm_blood = ch->max_blood;
		fMatch = TRUE;
		break;
	    }
	...
	case 'C':
	    ...
	    if (!str_cmp(word, "Chan")) {
		if (color >= MAX_CHANNELS) {
		    fread_to_eol(fp);
		    fMatch = TRUE;
		    break;
		}
		ch->pcdata->channel[color] = fread_string(fp);
		color++;
		fMatch = TRUE;
		break;
	    }
	...
	case 'G':
	    ...
	    KEY("God", ch->pcdata->god, fread_string(fp));
	...
	case 'I':
	    if (!str_cmp(word, "Ignore")) {	/* Ignore Code */
		if (count >= MAX_IGNORE) {
		    fread_to_eol(fp);
		    fMatch = TRUE;
		    break;
	...
	case 'Q':
	    KEY("Qwest", ch->pcdata->qwest, fread_number(fp));
	    break;

	case 'R':
	    KEY("Race", ch->race, race_lookup(fread_string(fp)));
		KEY("Rem", ch->remorts, fread_number(fp));
	...
	case 'S':
	    ...
	    KEY("Silv", ch->gold, fread_number(fp));
	    KEY("Smite", ch->pcdata->wrath, fread_string(fp));
	    KEY("Slo", ch->slots, fread_number(fp));

	    if (!str_cmp(word, "Slots") || !str_cmp(word, "Sl")) {
		int sn = 0;
		int value;
		char *temp;
		...
		if (sn < 0) {
		    bug("Fread_char: unknown slot. ", 0);
		} else {
		    ch->pcdata->slot[sn] = value;
		}
		fMatch = TRUE;
		break;
	    }
	...
	case 'W':
	    ...
	    KEY("Wait", ch->wait, fread_number(fp));
	    break;
	}

	if (!fMatch) {
	    bug("Fread_char: no match.", 0);
	    sprintf(buf, "word read was: %s", word);
	    bug(buf, 0);
	    fread_to_eol(fp);
	}
    }
...
void fread_pet(CHAR_DATA * ch, FILE * fp)
{
	...
	case 'H':
		...
		if (pet->hit >= pet->level * 15)
		    pet->hit = pet->level * 15;

		pet->max_hit = fread_number(fp);

		if (pet->max_hit >= pet->level * 15)
	...
	case 'S':
	    ...
	    KEY("Silv", pet->gold, fread_number(fp));
	    break;
...
void fread_obj(CHAR_DATA * ch, FILE * fp)
{
	...
	case 'A':
		...
		sn = skill_lookup(fread_word(fp));
		if (sn < 0)
		    bug("Fread_obj: unknown skill. %d", sn);
		if (sn == 152) {
		    obj->value[4] += 512;
		    bug("QUICKPOISON restored on item", 0);
		}
		paf->type = sn;
		bug("Skill is %d", sn);
		bug("QUICKPOISON restored on item", 0);
	...
	case 'V':
		...
		//bug("obj->value[4] is %d", obj->value[4]);
		//              if ( obj->value[4] - 512 >= 0)
		//                {
		//                bug("obj->value[4] is removed %d", obj->value[4]);
